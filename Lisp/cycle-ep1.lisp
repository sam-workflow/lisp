   ...
   (let ((counter 0) date P0  chan cr can  vri csignal cy2 trend bri ldev10
	...
	(ifn tdate (setq date (car (last (month-days (get-latest-index-date)))))(setq date tdate))
        ...
        (with-open-file (stream path1 :direction :output :if-exists :append :if-does-not-exist :create)
         (format stream "~A~%MKT      PR     CY1 CY2   BRI  VRI   CHAN   CR  CSIG   CAN  GANN  RSQR  MCD  LDEV L10~%" (date-convert date)))
        ...
        (setq chan  (channel-direction date 5))    
...
(defun cycles-d (date market &optional (min-period 10) (max-period 30) (outfile nil))
  (let (period0 dir0 dfl dfh )
      ...
      (when outfile
         (with-open-file (stream (string-append "/home/register/cycles/"
...
(defun time-cycle-ratios (&optional date0)
 (let ((filt *n-filt*) prices times proj-list t1 t2 t3 t4 t5
       ;(fibonacci-numbers '( 8 13 21 34 55 89  ))
...
(defun fibonacci-target-zones (&optional date0 )
 (let ((filt *n-filt*)  prices proj-list ignore p1 p2 p3 p4 p5 (counter 0)
       (fibonacci-numbers '(13 21 34   )) used 2high 2low days proj-list1)
...
(defun fibonacci-turns (&optional date0 )
 (let ((filt *n-filt*)  prices proj-list ignore p0 p1 p2 p3 p4 p5 (counter 0)
       (fibonacci-numbers '(55)) used  days ;proj-list1 ext
       tol)
  ...
  (let (proj-list (tol (pivot-tol tdate)) (ct 0) (phigh (getd tdate 'high))(plow (getd tdate 'low)))
    (multiple-value-setq (proj-list tol)(fibonacci-turns tdate))
       (setq proj-list (append (pivot-points1 tdate 'month); (zero-proj tdate 4)
                               (pivot-points1 tdate 'week);(channel-proj tdate 4)
                                proj-list))
       (if (= phigh (n-day-high tdate 5))
           (setq ct (- (count-if #'(lambda(s) (and (> phigh (- s tol))(< phigh (+ s tol)))) proj-list))))
       (if (= plow (n-day-low tdate 5))
...
(defun dominant-cycle (date &optional (min-size 10)(max-size 30))
    (let ((tdate (add-mkt-days date -20)) period  periods)
    (dotimes (ith 21) ;61)
...
|#
(defun day-trades4 (&optional tdate (market-list *fore-list*)  (holidayp nil))
...
;          (path-in (string-append *config-dir* "day-features4.dat"))
    ...
    ;	(ifn tdate (setq date (car (last (month-days (get-latest-index-date)))))(setq date tdate))
     ...
     ; (build-daytrade-warehouse4 *day-list* tdate)
     ; (apply #'day-trade-bins4b T features)   
      ;(apply #'day-trade-bins4a T features)   
...
(defun write-nt-edge-best-pf (num-markets)
 ...
 ;     (write-nt-edge-record ninja-output (fourth record) (fifth record) (sixth record)(seventh record) 0 0 )
   ...
   (dotimes (ith (min num-markets (length *ts-pf-list*)))
...
(defun ts-test (tdate)
 (let (date days); paths)
   ...
   (build-daytrade-warehouse4 *day-list*)
   (apply #'day-trade-bins4b T *day-features4*)   
  (dotimes (ith days)
  ; (setq *fore-list* *day-list*)

   (when (member ith (set-difference (month-days (round (/ ith 100))) *do-not-trade*))
...
;     (format stream "~A~%" (date-convert date1)))
    ...
    (day-trades4 tdate *day-list* holidayp)
...
(defun zero-strength (tdate size)
 ...
 (cond ((and (<= p0 z1)(< z0 z1 z2)) '3SU) ; 3-step
       ((and (>= p0 z1)(> z0 z1 z2)) '3SD) ; 3-step
       ((and (<= p0 z1)(< z-1 z0 z1)) 'LDU) ; last ditch
       ((and (>= p0 z1)(> z-1 z0 z1)) 'LDD) ; last ditch
       ((and (eql dir0 'UP)(< p0 z2)(< p0 z1)) 'WUP)
       ((and (eql dir0 'DN)(> p0 z2)(> p0 z1)) 'WDN)
       ((and (eql dir0 'UP)(> p0 z2)(> p0 z1)) 'SUP)
       ((and (eql dir0 'DN)(< p0 z2)(< p0 z1)) 'SDN)
       ((and (eql dir0 'UP)(<= p0 z1)) 'UPT)
       ((and (eql dir0 'DN)(>= p0 z1)) 'DNT)
       ((and (eql dir0 'UP)(> p0 z1)) 'AZT)
...
(defun zero-strength-swing345 (date)
  (let ((dir (parabolic-stops date)) (zb3 (zero-strength date 3))
       (zb4 (zero-strength date 4)) (zb5 (zero-strength date 5)))
(+
  (cond ((and (eql dir 'SHORT) (member zb3 '(WDN LDD BZT SDN))) 1)
        ((and (eql dir 'SHORT)(member zb3 '(3SD 3SU WUP))) 0)
        ((and (eql dir 'LONG)(member zb3 '(LDD LDU DNT))) -1)
        ((and (eql dir 'LONG) (member zb3 '(3SU WDN 3SD))) 0)
        (t 0))
 (cond ((and (eql dir 'SHORT) (member zb4 '(WDN LDD BZT SDN))) 1)
        ((and (eql dir 'SHORT)(member zb4 '(UPT LDU 3SU))) 0)
        ((and (eql dir 'LONG)(member zb4 '(LDU UPT))) -1)
        ((and (eql dir 'LONG) (member zb4 '(WDN SDN 3SD AZT))) 0)
        (t 0))
 (cond ((and (eql dir 'SHORT) (member zb5 '(WND BZT DNT SDN))) 1)
        ((and (eql dir 'SHORT)(member zb5 '(SUP 3SD WUP LDU))) 0)
        ((and (eql dir 'LONG)(member zb5 '(WUP WDN DNT LDU))) -1)
        ((and (eql dir 'LONG) (member zb5 '(SUP UPT SDN))) 0)
        (t 0))
)
...
;;;safe is 1.618 to the 1/5 power
;;;expand is 1.618 to the 1/5 power = 18 degrees on the log spiral
;;;PC-ADD-FACTOR aim says to add 50% of purchases to portfolio control
;;; 1.1010259 is safe
;;; abp is aim buy price
;;; abs is aim sell price

(defun aim-action (pc num-shares ftp cash)
  (let* ((safe .1010259) (contract (/ 1 (+ 1 safe)))(expand (+ 1 safe))
      	 (pc-add-factor .50)
	 (abp (* (^ expand 7) ftp)) abq (asp (* (^ contract 7) ftp)) asq
	 (minp (* safe pc)))

    ;first start with a very big unrealistic buy price
    ;then successively reduce the price until aim calls for buying more than the minimum amount minp
    ;; the minimum amount is in dollars and determined by the commission rate and the size of pc
    ;;; good number to use is minp = safe * pc.


    (loop
      (if (> (- pc (* abp num-shares) (* safe pc)) minp) (return))
      (setq abp (* contract abp))) ;;closes the loop

    (setq abq (/ (- pc (min cash (* abp num-shares)) (* safe pc)) abp))

 ;first start with a very small unrealistic sell price
 ;then successively increase the price until aim calls for selling more than the minimum amount minp
 ;; the minimum amount is in dollars and determined by the commission rate and the size of pc
 ;;; good number to use is minp = safe * pc.

    (loop

     (if (> (- (* asp num-shares) pc (* safe pc)) minp) (return))
     (setq asp (* expand asp)))

    (setq asq (/ (- (* asp num-shares) pc (* safe pc)) asp))
...
;			    )
    ...
    (values (my-round abp 2)
	    (ceiling abq)
	    (my-round asp 2)
	    (ceiling asq)

	    (round (+ pc (* pc-add-factor (ceiling abq)(my-round abp 2)))))
    ))

(defun elmoturn-dates (startdate enddate)
   (let ((date startdate) elmo-dates)
  (loop
   (if (member 0
          (get-elmo (format nil "~S" (add-mkt-days date -1000))(format nil "~S" date))) (push date elmo-dates))
   (if (eql date enddate)(return elmo-dates))
   (setq date (getd  date 'ndate)))
   ))
