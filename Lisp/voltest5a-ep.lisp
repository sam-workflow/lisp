...
(defparameter *charger-lite-list* '(cf.d1b hu.d1b si.d1b cl.d1b ho.d1b))
;;;
;;;;For DAY TRADING
;;;
...
(defun update-daytrade-warehouse3 (date &optional (markets *day-warehouse-list* ))
    ...
    (dolist (ith markets)
      (set-market ith)
       ...
       (populate-day-trades3 ith date (min 4371 (available-days ith date 500)))) 
 
    (build-daytrade-warehouse3 markets) 
    (setq *day-features3* (find-best-indicator-set3a )) 
    (indicator-study3)
   (portfolio-simulation3 '(day) date 4040 (list *day-warehouse-list*)) 
)
...
(defun populate-day-trades3 (market date2 num &optional (outfile T))
 (let (date trades long short cover-long cover-short entry-long entry-short date-1
       risk vri1 vri2 rri10 rri5 swi ccid 
  ...
  (setq *entry-factor* .764 *stop-loss-day* 1.0557 ;.944 ;  1.0557 
         *commission* 40 *max-day-risk* 2000 *pips-slippage* 0)  ;;;1.0557)
   (unless date2
...
;   (multiple-value-setq (stop-long stop-short)(vprices date 4 *stop-loss-day* 1))
   ...
   (setq vri2 (volatility-ratio-index date 4 28 1) vri1 (volatility-ratio-index date 1 7 1)
         rri10 (roc-rel-index date 10 90) rri5 (roc-rel-index5 date) swi (ww-swing-index1 date)
   ...
   (setq stop-long (-  entry-long  risk)
      ...
      ;  (setq  period (dominant-cycle date 10 30))
       ...
       (list *data-name* (getd date 'ndate) direction
             (and entry (my-pretty-price entry))
                             ...
                             (ww-swing-index1 date) ;;;feature 3
                           ;  (round (ww-swing-index date-1) 25)  ;;features 3
                             (oixdp date 11) ;feature 4      
                             (oixdp date 5) ;feature 5

                             (volatility-ratio-index date 4 28 1) ;feature 6 
                             
                             (volume-index date 4 28) ;feature 7 
                             (ldev-index date 5)  ;feature 8 
                             (lbounds date 5 2) ;; feature 9                          
                             (volume-ratio-index1 date 5 20 2) ;;feature 10
                                                      
                             (channel-direction-index date 7) ;;feature 11* 
                             (wpp-indexes-composite date ) ;feature 12*                      
                                                        
                             (volume-ratio-index1 date 5 20 2) ;;;feature 13* 2 day change                      
                             (candle-composite date 1) ;;;feature 14*                   
                              );;;closes the list
...
(defun replace-daytrade-indicators3 ()
  ...
  (dolist (ith daytrades)
     ...
     ; (setf (svref ith 5)     (ww-swing-index1 date-1));;;feature 3      

      (setf (svref ith 6)    (oixdp date-1 11)) ;feature 4      
      (setf (svref ith 7)    (oixdp date-1 5)) ;feature 5
  
   ;   (setf (svref ith 8)    (volatility-ratio-index date-1 4 28 1)) ;feature 6 
                             
   ;   (setf (svref ith 9)    (cci-direction date-1 21 2)) ;feature 7 
   ;   (setf (svref ith 10)   (asi-direction date-1)) ;feature 8 
   ;   (setf (svref ith 11)   (lbounds date-1 5 2)) ;; feature 9                          
      (setf (svref ith 12)   (volume-ratio-index1 date-1 5 20 2)) ;;feature 10
                                                      
   ;   (setf (svref ith 13)   (obv date-1 7)) ;;feature 11 
   ;   (setf (svref ith 14)   (wpp-indexes-composite date-1)) ;feature 12                           
                                                        
   ;   (setf (svref ith 15)   (volume-ratio-index1 date-1 5 20 2)) ;;;feature 13 2 day change                     
    ;  (setf (svref ith 16)   (candle-composite date-1 2)) ;;;feature 14*                   
...
(defun daytrade-simulation-test3 (market date2 num &optional (features nil))
 (let (date trades long short cover-long cover-short entry-long entry-short record trading-dates
        date-1 epsignal longs long-gains shorts short-gains (running-sum 0) risk vri1 vri2 rri10 rri5
       ave-win ave-loss losers winners extended-trades long-trade short-trade stop-long stop-short
       long-acc short-acc   bin draw ignore singles unfiltered-trades swi ccid
  ...
  (format T "~%~A~%" *data-name*)
   (setq *entry-factor* .764 *stop-loss-day* 1.0557 ;.944  ;1.0557
         *commission* 40 *min-day-expected-value* 80 *max-day-risk* 2000 *pips-slippage* 0) 
...
; (format T "Market= ~A date=~A~%" *data-name* date)
   ...
   (setq risk (volatility date *volatility-lookback* *stop-loss-day*))
   (setq vri2 (volatility-ratio-index date 4 28 1) vri1 (volatility-ratio-index date 1 7 1)
         rri10 (roc-rel-index date 10 90) rri5 (roc-rel-index5 date) swi (ww-swing-index1 date)
         ccid (cci-direction date 21 2))        
...
;;;check if new entry
   (when (and 
             ...
             (if (member *data-name* *charger-lite-list*) T (member epsignal '(OK DOWN)))
...
(defun display-unfiltered-trades (swings &optional (sdate 0)(str T))
   (let (training-data markets-list num-trades first-date last-date
    ...
    (dolist (jth markets-list)
     (format str"~%~10A ~5D ~9@D  ~4D   ~4,2F  ~8@D  ~A  ~A"
     (first jth) (second jth)(round (third jth)) (round (fourth jth))(fifth jth)
      (round (sixth jth))(seventh jth)(eighth jth)))
...
(defun daytrades-leave-one-out3a (base-features)
  (let (winners-list (result 0) rtb rp average-profit single-bins)
...
;   (setq base-list (append base-list candidate-list))
   (loop     
      ...
      (if (neql (second (car winners-list)) 1)
          (if (> (fifth (car winners-list)) 3.9)(return))
        (if (> (fifth (second winners-list)) 3.9)(return)));
  ...
  (let (contents (result 0) profit-list (all-winners 0)(all-losers 0)(twr 1)
   ...
   (dolist (ith longest-day-bin-codes)
    ...
    (dolist (kth daytrades)
     ...
     (format T "PROFIT PER TRADE IN WINNING BINS = ~A~%~%"
           (if (zerop num-in-winning-bins) 0 (round (/ winners num-in-winning-bins))))
     (format T "NUMBER OF BINS WITH ONLY 1 TRADE = ~D~%" only-one)
     (format T "RATIO OF ONLY 1 TRADE BINS TO ALL BINS = ~3,2F~%~%" (my-round (/ only-one (length longest-day-bin-codes)) 2))
     (format T "RATIO OF ONLY 1 TRADE BINS TO ALL TRADES = ~3,2F~%~%" (my-round (/ only-one (length daytrades)) 2))
  ...
  (let (contents (result 0) profit-list (all-winners 0)(all-losers 0) 
   ...
   (dolist (ith day-bin-codes)
    ...
    (dolist (kth daytrades)
     ...
     (format T "CHI SQUARE METRIC = ~D~%"  (daytrade-chi-squared-gof))
...
;;;here we remove any trades in the same market with the same next day
  ;  (format T "contents = ~A~% twr-long = ~A twr-short = ~A~%~%" contents twr-long twr-short)
   (setq contents
...
(defun find-best-daytrade3 (tdate &optional (output T)(output1 T)(date1 nil))
  (let (risk stop-short stop-long  entry-long entry-short  vri1 vri2 rri10 rri5 swi ccid
    ...
    (setq *entry-factor* .764 *stop-loss-day* 1.0557
          *commission* 40 *max-day-risk* 2000 ;2500
    ...
    (setq 
     ...
     (setq vri2 (volatility-ratio-index tdate 4 28 1) vri1 (volatility-ratio-index tdate 1 7 1)
           rri10 (roc-rel-index tdate 10 90) rri5 (roc-rel-index5 tdate) swi (ww-swing-index1 tdate)
           ccid (cci-direction tdate 21 2)) 
 
    (format T "~%EPSIGNAL = ~A VRI2= ~A VRI1= ~A~%  rri10=  ~A  rri5= ~A~%longs = ~A  long-gains = ~A ~%"
            epsignal vri2 vri1 rri10 rri5 longs long-gains )
...
;  (setq *ommission* 85 *max-day-risk* 2700)  ;;;1.0557)
  ...
  (setq *entry-factor* .764 *stop-loss-day* .944 *commission* 40 *max-day-risk* 1900)  ;;;1.0557)
  (unless date2
       (setq date2 (car (last (month-days (get-latest-index-date))))))
...
(defun currencies-simulation-test3 (market date2 num &optional (features nil))
 (let (date trades long short cover-long cover-short entry-long entry-short record trading-dates
   ...
   (format T "~%~A~%" *data-name*)
   (setq *entry-factor* .764 *stop-loss-day* .944 *commission* 40 *max-day-risk* 1900)  
...
(defun find-best-daytradeX (tdate &optional (output T)(date1 nil))
  (let (risk  stop-short stop-long  entry-long entry-short
   ...
   (setq risk (volatility tdate 4 *stop-loss-day*))
   (setq vri2 (volatility-ratio-index tdate 4 28 1) vri1 (volatility-ratio-index tdate 1 7 1)
         rri10 (roc-rel-index tdate 10 90) rri5 (roc-rel-index5 tdate))        
