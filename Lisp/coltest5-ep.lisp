...
(defparameter *volatility-lookback* 4);;;look back period 
...
(defun update-daytrade-warehouse3 (date &optional (markets  *day-warehouse-list* ))
    ...
    (dolist (ith markets)
      (set-market ith)
       (populate-day-trades3 ith date (available-days ith date))) 
 
    (build-daytrade-warehouse3 markets) 
    (setq *day-features3* (find-best-indicator-set3a )) 

   (portfolio-simulation3 '(day) date 3300 (list markets))
)
...
(defun populate-day-trades3 (market date2 num &optional (outfile T))
 (let (date trades long short cover-long cover-short entry-long entry-short date-1
  ...
  (setq *entry-factor* .618 *stop-loss-day* 1.382   ;.764 1.0557  ;.944
         *day-commission* 50 *max-day-risk* 2000)  ;;;1.0557)
   (unless date2
 ...
 (dotimes (ith num)
   ...
   (multiple-value-setq (entry-short entry-long)(vprices date *volatility-lookback* *entry-factor* 1))
...
;;;exit at the close
   ...
   (when (and (<= (getd date 'low) entry-short)
              ...
              (>= (* risk (index-point-value)) 75) ;;to avoid bad data
       ...
       (list *data-name* (getd date 'ndate) direction
             (and entry (my-pretty-price entry))
                   ;          (my-round (/ (volatility date 4 1) (volatility date 63 1)) 3) ;;;feature 2
                              (primitive-slope-trend date 3);;;feature 2
                              (ep-roc-change-index date 3 10) ;;feature 3 10 levels
                       ;       (round (slow-stochastic date 21))  ;feature 4
                              (candle date);;feature 4
                              (day-bar-type2 date) ;feature 5  7 levels
                              (cloud-index date) ;;;feature 6  8 levels
                              (roc-rel-index1 date 2 20) ;feature 7
                      ;        (gann-slope-index date 5 63);;feature 7  5 levels
                              (tkcd-index date ) ;feature 8  6 levels
                              (volatility-ratio-index date 1 14 1) ;; feature 9 6 levels
                              (volatility-ratio-index date-1 1 14 1);;;feature 10  6 levels
                              (day-bar-type2 date-1) ;feature 11  5 levels
                           ;   (daytrade-reward-risk date direction) ;;feature 12
                           ;   (read-from-string (day-of-week (getd date 'ndate)))      ;;  feature 12  7 levels
                              (rsi-cross date 9 3 );;;feature 12 
                           ;  (mo-diver date 45) ;;feature 13
                              (tsen date) ;feature 13  6  levels                              
...
(defun daytrade-simulation-test3 (market date2 num &optional (features nil))
 (let (date trades long short cover-long cover-short entry-long entry-short record trading-dates
       ...
       ave-win ave-loss losers winners extended-trades long-trade short-trade stop-long stop-short
       long-acc short-acc  (twr-long 1) (twr-short 1) bin draw ignore singles;draw-ave draw-90
  ...
  ; (declare (ignore long-acc short-acc))
   ...
   (setq *entry-factor* .618 *stop-loss-day* 1.382 ; .764 1.0557 ;.944
           *day-commission* 50 *min-day-expected-value* 75 *max-day-risk* 2000) 
...
;;;check if new entry
   ...
   (when (and (> (getd date 'high) entry-short)
              ...
              (>= (/ short-gains shorts) *min-day-expected-value*) (> twr-short 1.0)
...
;              (>= (/ short-gains shorts)
 ;                 (if (> longs 0.0) (/ long-gains longs) 0))
    ...
    (when (and (< (getd date 'low) entry-long)
               ...
               (>= (/ long-gains longs) *min-day-expected-value*) (> twr-long 1.0)
               (>= (* risk (calculate-point-value date-1)) 75)
	       (<= (* risk (calculate-point-value date-1)) *max-day-risk*)
  ;             (> (/ long-gains longs)
...
(defun find-best-indicator-set3 (markets &optional (period 3500))
  (let (base-list candidate-list winners-list (result 0) tdate best-features
        idct-list dmy one-only winners-ratio  (pathout (string-append *config-dir* "day-features3.dat")))
...
;                 (push record best-features))))
    ...
    (loop
     ...
     (if (or (not candidate-list)(> result .236))
         (return))
    )
   ...
   (loop     
      ...
      (if (< (fourth (car winners-list)) .146)(return))
...
;;now letsuse the above indicators to find the best overall set iwth one-one trade bins less than 10% of all trades
   (setq idct-list (indicator-count pathout))
   (format T "~%~A~%" idct-list)
   (setq idct-list (remove-if #'(lambda (s) (= (cdr s) 0)) idct-list)) ;;;removes indicators not used
   (setq idct-list (reverse (mapcar #'(lambda(s) (car s)) idct-list)))
   (setq idct-list (remove 1 idct-list))
   (setq idct-list (cons 1 idct-list));;;idct is a list of indicators in order of importace based on frequency of occurance

   (dotimes (ith (length idct-list))
      (setq dmy (append dmy (list (nth ith idct-list))))    
      (multiple-value-setq (ignore ignore ignore one-only winners-ratio)(apply #'day-trade-bins3a dmy))
      (if (or (>= one-only .236)(>= winners-ratio .764))(return))
...
(defun find-best-indicator-set3a ()
  (let (base-list candidate-list winners-list (result 0)); tdate best-features)
    ...
    (loop
     ...
     (if (or (not candidate-list)(> result .96))
         (return))
    )
    (format T "First stage winners-list= ~A"  winners-list)

   (loop     
      (if (< (fourth (car winners-list)) .84)(return))
...
(defun rank-daytrade-bins-by-profit3b (daytrades features)
  (let (contents (result 0) profit-list (all-winners 0)(all-losers 0)(twr 1)
   ...
   (dolist (ith longest-day-bin-codes)
    ...
    (dolist (kth daytrades)
     ...
     (format T "NUMBER of ALL BINS = ~D~%" (length longest-day-bin-codes))
     (format T "RATIO OF TRADES TO BINS = ~F~%" (my-round (/ (length daytrades)(length longest-day-bin-codes)) 1))
     ...
     (format T "RATIO OF ONLY 1 TRADE BINS TO ALL BINS = ~F~%~%" (my-round (/ only-one (length longest-day-bin-codes)) 2))
  ...
  (let (contents (result 0) profit-list (all-winners 0)(all-losers 0)(twr 1.0d0)
   ...
   (dolist (ith day-bin-codes)
    ...
    (dolist (kth daytrades)
     ...
     (format T "RATIO OF ONLY 1 TRADE BINS TO ALL TRADES = ~F~%" (my-round (/ only-one (length daytrades)) 2))
     (format T "CHI SQUARE PROB = ~7,5F~%" (my-round (daytrade-chi-squared-gof) 5))

    (values (round winners)(round (+ all-winners all-losers))(if (zerop num-in-winning-bins) 0 (round (/ winners num-in-winning-bins)))
           (my-round (/ only-one (length daytrades)) 2)(my-round (/ winners all-winners) 2) (daytrade-chi-squared-gof)) 
...
(defun display-daytrade-bins-by-expected-value3 ()
  (let (contents expected-value-list result (winners 0)(losers 0) shorts longs
          (path1 (string-append *output-upper-dir* "daytrade-expected-value3.dat")))
   (dolist (ith day-bin-codes)
     (setq contents
...
(defun display-num-trades-by-market3 (swings)
   (let (training-data markets-list num-trades first-date last-date
    ...
    (dolist (ith training-data)
      ...
      (setq markets-list
           (cons (list ith num-trades pl ave-pl pf draw first-date last-date) markets-list))
       );;closes the dolist
   ...
   ; (vsort markets-list #'< #'(lambda(s)(first-char-code (car s))))
    (with-open-file (str path :direction :output :if-exists :supersede :if-does-not-exist :create)
     (format str "Market    #trades   $P&L   $Ave    PF    Drawdown    FIRST     LAST~%")
    (dolist (jth markets-list)
     (format str"~%~10A ~4D ~10D   ~4D    ~A  ~A ~A  ~A"
     (first jth) (second jth)(third jth) (fourth jth)(fifth jth)(sixth jth)(seventh jth)(eighth jth))))
  ));;closes the let and defun.

(defun gain-loss-trades-in-warehouse3 (market swings)
  (let ((pl 0) (losses 0) (gains 0) draw trades)
     (dolist (record swings)
       
       (when (eql market (svref record 0))
             (setq pl (+ pl (svref record 19)) trades (cons record trades))
             (if (plusp (svref record 19)) (setq gains (+ gains (svref record 19)))
                (setq losses (+ losses (svref record 19))))
        ))
...
(defun find-best-daytrade3 (tdate &optional (output T)(output1 T)(date1 nil))
  (let (risk  stop-short stop-long  entry-long entry-short
        ...
        action directive1 cannon-starter-path3 cannon-score-path3 foremost-starter-path3 
         foremost-score-path3  kingsview-starter-path3 striker-starter-path3 striker-score-path3
         striker-entry-path3 striker-charger-path3
    ...
    (setq *entry-factor* .764 *stop-loss-day* 1.309 *day-commission* 50 *max-day-risk* 2000) ;;;  1.0557)
    ...
    (setq cannon-score-path3 (string-append *daily-output-dir* "cannon-score.csv")
          ...
          striker-charger-path3 (string-append *daily-output-dir* "striker-charger.csv")
  ...
  (format T "~%EPSIGNAL = ~A~%  longs = ~A  long-gains = ~A twr-long = ~7,5F~%" epsignal longs long-gains  twr-long)
  ...
  (setq risk (volatility tdate 4 *stop-loss-day*))
   ...
   (if (and (member epsignal '(OK UP))(> longs 0)(<= (* risk (calculate-point-value tdate)) *max-day-risk*)
            (>= (/ long-gains longs) *min-day-expected-value*)(> twr-long 1.0))
         (push 'UP trade-direction)(push 'FT trade-direction))
   (if (and (member epsignal '(OK DOWN))(> shorts 0)(<= (* risk (calculate-point-value tdate)) *max-day-risk*)
            (>= (/ short-gains shorts) *min-day-expected-value*)(> twr-short 1.0))
...
;                                  (index-point-value))) *max-day-risk*)) (push "NOT TODAY" action))
  ...
  ;                            central-cancel-time central-exit-time central-end-session-time oco-code)
             ...
             (when (member *data-name* *charger-list*)
                 (setq offset (random-choice -1 0)
                    pacific-exit-time (string-append (date-convert date1) " " (add-minutes1 (third (assoc *data-name* *cannon-market-times-list*)) offset T)))
...
;               (write-oec-short rjobrien-starter-path3 oec-symbol *rjobrien-starter-block-acct* *rjobrien-starter-qty* entry-short stop-short central-entry-time
 ;                               central-cancel-time central-exit-time central-end-session-time oco-code)
  ...
  ;                             central-cancel-time central-exit-time central-end-session-time oco-code)
               ...
               (write-oec-short striker-charger-path3 oec-symbol *striker-charger-block-acct* *striker-charger-qty* entry-short stop-short central-entry-time
                                central-cancel-time central-exit-time central-end-session-time oco-code)
               (setq oco-code (format nil "OSO~A" (incf counter)));;need to add 1 to the OCO code if both buy and sell entries
               (write-oec-long striker-charger-path3 oec-symbol *striker-charger-block-acct* *striker-charger-qty* entry-long stop-long central-entry-time
                               central-cancel-time central-exit-time central-end-session-time oco-code)
...
;
 ;               (write-oec-long daniels-starter-path3 oec-symbol *daniels-starter-block-acct* *daniels-starter-qty* entry-long stop-long pacific-entry-time
  ...
  ;                              central-cancel-time central-exit-time central-end-session-time oco-code)
                ...
                (write-oec-long striker-charger-path3 oec-symbol *striker-charger-block-acct* *striker-charger-qty*
                                entry-long stop-long central-entry-time
                                central-cancel-time central-exit-time central-end-session-time oco-code)
...
(defun replace-daytrade-indicators3 ()
  ...
  (dolist (ith daytrades)
    ...
    ; (setf (svref ith 14) (roc-rel-index date-1 9 84));;;feature 12
  ;  (setf (svref ith 6) (candle date-1));;feature 4
      (setf (svref ith 4) (primitive-slope-trend date-1 5));;feature 2
  ;  (setf (svref ith 7)(num-waves date-1)) ;;feature 5
   ;  (setf (svref ith 11) (range-index date-1 ));;feature 9
   ;  (setf (svref ith 12)(macd-index1 date-1 6 15 3 84));;feature 10
   ;   (setf (svref ith 15)(tsen date-1));;feature 13
...
(defun update-currencies-warehouse3 (date &optional (markets *currencies-list*))
   ...
   (mapcar #'(lambda (s1) (populate-currencies-trades3 s1 date (cdr (assoc s1 *market-days-available*)))) markets)
   (build-currencies-warehouse3 markets)   
   (setq *currencies-features3*  (find-best-indicator-set3c ))
...
(defun populate-currencies-trades3 (market date2 num &optional (outfile T))
 (let (date trades long short cover-long cover-short  date-1
  ...
  (format outfile "~%~A~%" market)
;  (setq *day-commission* 85 *max-day-risk* 2700)  ;;;1.0557)
 
  (setq *entry-factor* .764 *stop-loss-day* .944 *day-commission* 75 *max-day-risk* 1900)  ;;;1.0557)
  (unless date2
       (setq date2 (car (last (month-days (get-latest-index-date))))))
       ...
       (list *data-name* (getd date 'ndate) direction
             (and entry (my-pretty-price entry))
                   ;          (my-round (/ (volatility date 4 1) (volatility date 63 1)) 3) ;;;feature 2
                              ...
                              (rsi-ave-diff-index date 14 2) ;;feature 3 5 levels
                       ;       (round (slow-stochastic date 21))  ;feature 4
                              (slow-stochastic-index date 21);;feature 4
                              (day-bar-type2 date) ;feature 5  10 levels
                              (macd-index date 12 26 9) ;;;feature 6  5 levels
...
(defun currencies-simulation-test3 (market date2 num &optional (features nil))
 (let (date trades long short cover-long cover-short entry-long entry-short record trading-dates
   ...
   (set-market market)
   (format T "~%~A~%" *data-name*)
   (setq *entry-factor* .764 *stop-loss-day* .944 *day-commission* 75 *max-day-risk* 1900)  
...
;;;check if new entry
   ...
   (when (and (> (getd date 'high) entry-short)
              (<= (getd date 'low) entry-short)
              (member epsignal '(OK DOWN))(> shorts 0.0)
              (>= (/ short-gains shorts) 75.0) (> twr-short 1.0)
	      (>= (* risk (calculate-point-value date-1)) 75)
              (<= (* risk (calculate-point-value date-1)) *max-day-risk*)
                 )
          (setq short entry-short
    ...
    (when (and (< (getd date 'low) entry-long)
               (>= (getd date 'high) entry-long)
               (member epsignal '(OK UP))(> longs 0.0)
               (>= (/ long-gains longs) 75.0) (> twr-long 1.0)
               (>= (* risk (calculate-point-value date-1)) 75)
	       (<= (* risk (calculate-point-value date-1)) *max-day-risk*)
             )
...
;  (multiple-value-setq (entry-short entry-long) (vprices date 4 *entry-factor-swing* 1))
;  (multiple-value-setq (prev-entry-short prev-entry-long) (vprices date-1 4 *entry-factor-swing* 1))

  (setq risk (min (volatility date 4 *stop-loss-swing*)
                   (/ *max-swing-risk* (calculate-point-value date))))
...
;   (setq risk (min (volatility date 21 *stop-loss-day*)(/ *max-swing-risk* (index-point-value))))
...
(defun find-best-indicator-set1a ()
  (let (base-list candidate-list winners-list (result 0)); tdate best-features)
    ...
    (loop
     ...
     (setq candidate-list (remove (second (car winners-list)) candidate-list))
     (setq result (fourth (car winners-list)));;;
     (if (or (not candidate-list)(> result .95))
         (return))
    )
    (format T "First stage winners-list= ~A"  winners-list)

   (loop     
      (if (< (fourth (car winners-list)) .85)(return))
