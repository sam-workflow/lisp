...
(defun available-days (market tdate &optional (delta 300))
...
(defun channel-trend (tdate size)
  (let* ((filt *n-filt*)(time-interval *time-interval*)(days (* size -15))
...
;;;these are the channel-projections

;;;cycle projected turns 3 periods out
(defun cycle-proj (tdate &optional (days -504)(min-period 10)(max-period 30))
  (let ((n-filt *n-filt*) (starttime (conv-to-string (add-mkt-days tdate days) 'A)) prices index-list
   ...
   (setq *time-interval* 'daily-high-low period *n-filt*)
    ...
    (push (truncate (- (+ (car index-list) (* 2 (1+ ith) *n-filt*)) *end-index*) 2) turns)
    (push (truncate (- (+ (cadr index-list) (* 2 (1+ ith) *n-filt*)) *end-index*) 2) turns))
   ; (format t "~% ~A ~A ~A ~A ~%" (car index-list) (* (1+ ith) *n-filt*) *end-index* turns)
    )
  (setq *n-filt* n-filt)
  (setq turns (remove-if #'(lambda(s)(or (< s -1)(> s 1))) turns))
  (values (car turns) period) 
))
...
(defun channel-trend1 (tdate size)
  (let ((filt *n-filt*)(time-interval *time-interval*)
         ...
         target (starttime (conv-to-string (add-mkt-days tdate (* size -25)) 'A)))    
    ;    (declare (ignore ignore))
        (setq *n-filt* size *time-interval* 'daily-high-low)(nil-tpv) 
        (multiple-value-setq (prices ignore ignore ignore index-list)
            (find-all-primitives starttime  (conv-to-string tdate 'P)))        
...
;     (print proj-list)
       ...
       (length proj-list1);;; 0 means no projection 1 means one projection
                        ;;; 2 means projections
...
(defun pivot-turn (date &optional (typ 'week))
  (let ((phigh (getd date 'high))(plow (getd date 'low))(pclose (getd date 'close))
   ...
   (setq pivots (pivot-points1 date typ) tol (pivot-tol date) 
         whigh (case typ
                (week  (second (current-week-high-low date)))
                (month (second (current-month-high-low date)))
                (day phigh))
         wlow (case typ 
                (week (first (current-week-high-low date)))
                (month (first (current-month-high-low date)))
                (day plow))
        )
...
(defun pivot-turn-composite (tdate days)
  (let (piv (date tdate) (result 0))
   
         (dotimes (ith days)
            (setq piv (pivot-turn date 'month))
...
#|
(defun roc-index (tdate size &optional (period 252))
  (let ((date tdate) dmy proc)
   (dotimes (ith period)
      (push  (roc date size) dmy)
      (setq date (getd date 'ydate)))
   (vsort dmy #'< )
   (setq proc (percentile1 (roc tdate size) dmy))
   (cond ((< proc .05) -4)
         ((and (>= proc .05)(< proc .10)) -3)
         ((and (>= proc .10)(< proc .20)) -2)
         ((and (>= proc .20)(< proc .50)) -1)
         ((and (>= proc .50)(< proc .80)) 1)
         ((and (>= proc .80)(< proc .90)) 2)
         ((and (>= proc .90)(< proc .95)) 3)
         ((>= proc .95) 4))
...
(defun accel (date size1 &optional (size2 1) (typ 'pivot))
  (- (ep-roc date size1 (* 100 size1) typ) (ep-roc (add-mkt-days date (- size2)) size1 (* 100 size1) typ)))
...
(defun accel-context (date size1)
  (let ((trend (roc date size1 'close))
        (acc (accel date size1 1 'close)))
...
;;;returns the number of waves in the privious price move at filter 11
...
(defun gann-slope-index (date0 &optional (filter 11) (period 63))
...
(defun gann-slope-ratio (date0 &optional (filter 11) (period 63))
